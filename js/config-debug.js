define("/DigiShell/js/config-debug", [ "./sidebar-debug", "backbone/backbone/1.1.0/backbone-debug", "jquery-debug", "backbone/underscore/1.5.2/underscore-debug", "./util-debug", "./widgets-debug", "jquery/bootstrap/fuelux/tree-debug", "jquery/gridster/0.2.1/jquery.gridster-debug" ], function(require, exports) {
    var sidebar = require("./sidebar-debug"), widgets = require("./widgets-debug"), Backbone = require("backbone/backbone/1.1.0/backbone-debug"), _ = require("backbone/underscore/1.5.2/underscore-debug"), util = require("./util-debug");
    require("jquery/bootstrap/fuelux/tree-debug");
    require("jquery/gridster/0.2.1/jquery.gridster-debug");
    ...
    DataSourceTree.prototype.data = function(options, callback) {
        function getParents(selectedItem, items) {
            ...
            var parentId = selectedItem.get("parentId"), parentIds = [], parent;
            if (parentId !== "root") {
        ...
        var items = this._items, selected = this._selectedItems, parentIds = selected ? function() {
            var parentIds = [];
        ...
        if (!("name" in options) && !("type" in options)) {
            ...
            _.each(items.where({
                ...
                result[item.id] = _.extend({}, item.attributes, {
                    type: type,
                    checked: type === "folder" ? null : selected ? !!selected.findWhere({
                        id: item.id
                    }) : false,
                    expanded: type === "folder" ? _.indexOf(parentIds, item.id) !== -1 : null
                });
    ...
    exports.init = function() {
        function addWidget(item) {
            var widget = widgets.getWidget(item.wId, item), widgetNode = widget.$el.clone();
        ...
        if (!initFlag) {
            ...
            var hotForm = sidebar.openMenu("hotForm"), hotApp = sidebar.getMenu("hotApp"), hotFormMap = sidebar.getSiteMap("hotFormMap", {
                el: $("#config_hotFormConfig_Tab")[0],
                selectable: true,
                selected: hotForm.model
            }), hotAppMap = sidebar.getSiteMap("hotAppMap", {
                el: $("#config_hotAppConfig_Tab")[0],
                selectable: true,
                selected: hotApp.model
            }), gridster;
            $("#config_hotFormConfig_Tab").on("select", function(event, item, items) {
                var menuData = [ {
                    id: "root",
                    level: 0
            ...
            $("#config_hotAppConfig_Tab").on("select", function(event, item, items) {
                var menuData = [ {
                    id: "root",
                    level: 0
                } ].concat(_.map(items, function(item) {
                    ...
                    if ($("#config_WidgetConfig_Tab .gridster li").size() === 0) {
                        ...
                        gridster = $("#config_WidgetConfig_Tab .gridster > ul").gridster({
                            ...
                            serialize_params: function($w, wgd) {
                                return _.extend($w.data("widgetConfig"), {
                                    col: wgd.col,
                                    row: wgd.row,
                                    size_x: wgd.size_x,
                                    size_y: wgd.size_y
            ...
            $("#config_WidgetConfig_Tab .gridster").on("click", function() {
                //阻止点击事件
                return false;
            });
            hotFormMap.getItem("root", function(item, items) {
                ...
                $("#config_WidgetConfig_Tab .tree").ace_tree({
                    ...
                    var widgetNodes = $(".gridster .gs_w"), removeWidgets = widgetNodes.filter(function() {
                        return !_.findWhere(data.info, {
                            id: $(this).data("widgetConfig").id
                        });
